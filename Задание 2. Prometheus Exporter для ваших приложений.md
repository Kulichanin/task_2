...
На прошлом занятии мы уже развернули в кластере kubernetes Prometheus Stack. Этот стек включает в себя Prometheus, Grafana, Alertmanager и, что самое важное, [Prometheus Operator](https://prometheus-operator.dev/docs/getting-started/introduction/). Prometheus Operator упрощает управление мониторингом, вводя специальные Custom Resource Definitions (CRD). Теперь наша задача — настроить мониторинг для приложения, чтобы его метрики собирались этим стеком. На этом уроке ты узнаешь, как это сделать.
...
Для большинства случаев используется ServiceMonitor, особенно для мониторинга стандартных сервисов, поэтому в практическом примере будет рассмотрен он. Для полноты картины пример манифеста ServiceMonitor.
...
Написание кода для сбора метрик для приложения выходит за рамки данного курса. Если вы столкнулись с тем, что приложение уже есть, а метрики для него нет, [вот тут](https://prometheus.io/docs/instrumenting/clientlibs/) в документации указаны библиотеки для популярных языков программирования.
...
Важно понимать, что данная команда работает в случае, если вы установили **prometheus** с помощью **kube-prometheus-stack** в namespace **monitoring**. Скорректируйте команду согласно вышим условиям работы prometheus.

Дополним переменные для servicemonitor в конец файла `whoami-chart/values.yaml`.
...
Откроется меню с доступными dashboard. Выберем слева из выпадающего меню **New** создание нового dashboard.
